  ...
  return (
    <section className="py-10 md:py-14 min-h-[70vh]">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight">По применению</h1>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {applications.map((app) => (
            <Link
              key={app.title}
              to="/contacts#lead"
              className="group rounded-2xl border border-slate-200 p-4 sm:p-5 hover:shadow-md bg-white transition-shadow"
            >
              <div className="mb-3 h-20 sm:h-24 w-full rounded-xl bg-gradient-to-br from-indigo-50 to-blue-50 border border-slate-100" aria-hidden="true"></div>
              <p className="text-sm sm:text-base font-medium group-hover:text-slate-900">{app.title}</p>
              <p className="text-xs sm:text-sm text-slate-600 mt-1">{app.description}</p>
            </Link>
          ))}
        </div>
      </div>
